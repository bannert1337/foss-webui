...
export const toggleModelsById = async (token: string, ids: string[], active: boolean) => {
	let error = null;

	const res = await fetch(`${WEBUI_API_BASE_URL}/models/bulk/toggle`, {
		method: 'POST',
		headers: {
			Accept: 'application/json',
			'Content-Type': 'application/json',
...
export const updateModelsById = async (token: string, ids: string[], updates: object) => {
	let error = null;
