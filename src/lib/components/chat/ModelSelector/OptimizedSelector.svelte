<script lang="ts">
	...
	import { createEventDispatcher, onMount, getContext, tick, onDestroy } from 'svelte';
	...
	// Create Fuse instance only once and update it when items change
	let fuse;
	$: {
		const fuseOptions = {
		...
		// Only recreate fuse instance when items change
		fuse = new Fuse(
			items.map((item) => {
				return {
					...item,
	...
	let filteredItems = [];
	$: {
		// Only recompute when these values change
		if (
			debouncedSearchValue !== undefined ||
			selectedTag !== undefined ||
			selectedConnectionType !== undefined
		) {
			filteredItems = getFilteredItems(debouncedSearchValue, selectedTag, selectedConnectionType);
		}
	}

	// Extract filtering logic to a function to avoid recomputing unnecessarily
	function getFilteredItems(search, tag, connectionType) {
		let result = [];

		if (search) {
			result = fuse.search(search).map((e) => e.item);
		} else {
			result = [...items];
		}
		...
		if (tag) {
			result = result.filter((item) => (item.model?.tags ?? []).map((t) => t.name).includes(tag));
		}

		// Filter by connection type if selected
		if (connectionType) {
			if (connectionType === 'ollama') {
				result = result.filter((item) => item.model?.owned_by === 'ollama');
			} else if (connectionType === 'openai') {
				result = result.filter((item) => item.model?.owned_by === 'openai');
			} else if (connectionType === 'direct') {
				result = result.filter((item) => item.model?.direct);
			}
		}

		// Filter out hidden models
		return result.filter((item) => !(item.model?.info?.meta?.hidden ?? false));
	}

	// Reset view with optimized DOM operations
	...
	// Process tags only once on mount
	onMount(async () => {
		ollamaVersion = await getOllamaVersion(localStorage.token).catch((error) => false);

		if (items) {
			// Process tags only once
			const allTags = items
			...
			tags = Array.from(new Set(allTags)).sort((a, b) => a.localeCompare(b));
		}
	});
	...
	function renderModelItem(item, index) {
		...
		const isHighlighted = index === selectedModelIdx;
		...
		return `
			<button
				...
				class="flex w-full text-left font-medium line-clamp-1 select-none items-center rounded-button py-2 pl-3 pr-1.5 text-sm text-gray-700 dark:text-gray-100 outline-hidden transition-all duration-75 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg cursor-pointer data-highlighted:bg-muted ${
					isHighlighted ? 'bg-gray-100 dark:bg-gray-800 group-hover:bg-transparent' : ''
				}"
				data-arrow-selected="${isHighlighted}"
				data-value="${item.value}"
			>
				<div class="flex flex-col">
					${
						$mobile && (item?.model?.tags ?? []).length > 0
							? `
					...
					<div class="flex items-center gap-2">
						...
						${
							!$mobile && (item?.model?.tags ?? []).length > 0
								? `
								<div class="flex gap-0.5 self-center items-center h-full translate-y-[0.5px] overflow-x-auto scrollbar-none">
									${item.model?.tags
										.sort((a, b) => a.name.localeCompare(b.name))
										.map(
											(tag) => `
	...
	>
		<slot>
			...
			<div class="px-3 max-h-64 overflow-y-auto scrollbar-hidden group relative">
				...
				<!-- Use VirtualList for efficient rendering -->
				{#if filteredItems.length > 0}
					<VirtualList items={filteredItems} height="250px" itemHeight={40} let:item let:index>
						<button
							aria-label="model-item"
